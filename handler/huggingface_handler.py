...
class HuggingfaceHandler(BaseHandler):
    def __init__(self, model_name: str, sample_rate=16_000):
        self.model_name = model_name
        self.model = AutoModel.from_pretrained(model_name, trust_remote_code=True)
        self.prev_outs_cache = defaultdict(None)
    ...
    def generate_stream(self, request):
        ...
        curr_hash = self._hash_message_content(messages)
        prev_hash = (
            self._hash_message_content(messages[:-2]) if len(messages) > 2 else None
        )
        ...
        for content in messages[-1]["content"]:
            if content["type"] == "audio":
                audio_str = content["audio_url"]
                y, sr = librosa.load(get_file_from_any(audio_str), sr=self.sample_rate)
                current_audio = y.astype(np.float32)
                current_audio /= np.max(np.abs(y))
                break
